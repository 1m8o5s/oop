...
int main(){
	...
	Cassa cs1;
	Cassa cs2(cs1);
	Cassa cs3 = {100, 10, false, 0, 10, 20, 0, "Arsen"};
	...
	SingerManager* sm = new SingerManager(0, 0, "Askold");
	Singer* sn = new Singer;
	SecurityManager* security = new SecurityManager(0, 0, "Eldar");
	cl->agreeCashPercentWithSingerManager(cs, sm, 20);
	cl->agreeTicketPriceWithSingerManager(cs, sm, 50);
	sm->setCityForConcert(concert1, sn, "Lviv");
