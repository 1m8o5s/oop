...
int main(){
	...
	Cassa cs1;
	Cassa cs2(cs1);
	Cassa cs3 = {100, 10, false, 0, 10, 20, 0, 0, (char*)"West Cassa"};
	...
	cout << endl << "visitors money after " << visitors[0].getDiscount() << endl;
	for (int i = 0; i < 5; i++)
		visitors2[i].goToConcert(concert1);
	std::cout << "Concert ticket price " << concert1->getTicketPrice() << endl;
