...
void MiniWorld::addSecurityManager(char* file, char*mode) {
	security_managers.push_back(new SecurityManager<int>());
	if (strcmp(mode, "binary")) {
		ServiceRead<SecurityManager<int>*>(security_managers[security_managers.size() - 1], file, false);
	}
	else {
		ServiceRead<SecurityManager<int>*>(security_managers[security_managers.size() - 1], file, true);
	}
}
...
MiniWorld::MiniWorld(vector<Cassa*>& cassa, vector<Visitor*>& visitors, vector<Concert*>&concerts, vector<SingerManager*>&singer_managers, vector<ClubManager*>&club_managers, vector<SecurityManager<int>*>&security_managers, vector<Singer*>&singers, int second, int minute, int hour, int day, int mounth, int year) 
...
void MiniWorld::executeCommands(stringstream& text) {
	...
	if (!strcmp(tmp_token, "tick")) {
		...
		else if (!strcmp(type, "SecurityManager<int>")) {
			removeComponent<vector<SecurityManager<int>*> >(security_managers, name);
		}
