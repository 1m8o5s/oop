...
void MiniWorld::addSingerManager(char* file, char*mode) {
	SingerManager *sing_manager = new SingerManager();
	if (strcmp(mode, "binary")) {
		ServiceRead<SingerManager>(*(sing_manager), file, false);
	}
	else {
		ServiceRead<SingerManager>(*(sing_manager), file, true);
	}
	singer_managers.insert(sing_manager);
}
void MiniWorld::addClubManager(char* file, char*mode) {
	ClubManager * club_man = new ClubManager();
	if (strcmp(mode, "binary")) {
		ServiceRead<ClubManager>(*club_man, file, false);
	}
	else {
		ServiceRead<ClubManager>(*club_man, file, true);
	}
	club_managers.insert(club_man);	
}
void MiniWorld::addSecurityManager(char* file, char*mode) {
	security_managers.insert({ security_managers.size(), new SecurityManager() });
	if (strcmp(mode, "binary")) {
...
void MiniWorld::addSinger(char* file, char*mode) {
	singers.insert({ singers.size(), new Singer() });
	map<int, Singer*>::reverse_iterator it = singers.rend();
	if (strcmp(mode, "binary")) {
		ServiceRead<Singer>(*it->second, file, false);
	}
	else {
		ServiceRead<Singer>(*it->second, file, true);
	}
}
...
void MiniWorld::removeConcert(char*title) {
	for (deque<Concert*>::iterator it = concerts.begin(); it != concerts.end(); ++it) {
...
void MiniWorld::removeSingerManager(char*name) {
	...
	for (set<SingerManager*>::iterator it = singer_managers.begin(); it != singer_managers.end(); ++it) {
		if (!strcmp((*it)->getName(), name)) {
...
void MiniWorld::removeClubManager(char*name) {
	for (multiset<ClubManager*>::iterator it = club_managers.begin(); it != club_managers.end(); ++it) {
		if (!strcmp((*it)->getName(), name)) {
...
void MiniWorld::removeSecurityManager(char*name) {
	for (map<int, SecurityManager*>::iterator it = security_managers.begin(); it != security_managers.end(); ++it) {
...
void MiniWorld::removeSinger(char*name) {
	for (multimap<int, Singer*>::iterator it = singers.begin(); it != singers.end(); ++it) {
		if (!strcmp((it)->second->getName(), name)) {
			singers.erase(it);
			break;
		}
...
MiniWorld::MiniWorld(vector<Cassa*>& cassa, list<Visitor*>& visitors, deque<Concert*>&concerts, set<SingerManager*>&singer_managers, multiset<ClubManager*>&club_managers, map<int, SecurityManager*>&security_managers, multimap<int, Singer*> &singers, int second, int minute, int hour, int day, int mounth, int year)
