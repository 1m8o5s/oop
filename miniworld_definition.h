...
private:
	...
	vector<SecurityManager<int>*> security_managers;
...
public:
	...
	SecurityManager<int>* getSecurityManager(char* name);
	Singer* getSinger(char* name);

	MiniWorld();
	MiniWorld(vector<Cassa*>& cassa, vector<Visitor*>& visitors, vector<Concert*>&concerts, vector<SingerManager*>&singer_managers, vector<ClubManager*>&club_managers, vector<SecurityManager<int>*>&security_managers, vector<Singer*>&singers, int second, int minute, int hour, int day, int mounth, int year);
