...
void Visitor::readDataFieldsFromFile(char* file){
	...
	bool vip_member;
	int number_of_place;
	fin >> cache >> years_old >> name >> discount >> have_ticket >> nickname >> vip_member >> number_of_place;
	this->setCache(cache);
...
void Visitor::writeDataFieldsToFile(char* file){
	...
	fout << this->getCache() << " " << this->getOld() << " " << this->getName() << " " << this->getDiscount() << " " << this->getTicketOwnership() << " " << this->getNickname() << " " << this->getMember() << " " << this->getNumberOfPlace();
	fout.close();
}
