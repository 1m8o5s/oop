...
void Concert::readDataFieldsFromFile(ifstream& file){
	...
	file >> artist_what_shown >> date_of_concert >> ticket_price >> age_limit >> city_for_concert >> earned_money >> number_of_visitors >> vip_member_number;
...
void Concert::writeDataFieldsToFile(ofstream& file){
	file << this->getTitle() << " " << this->getDateOfConcert() << " " << this->getTicketPrice() << " " << this->getAgeLimit() << " " << this->getCityForConcert() << " " << this->getEarnedMoney() << " " << this->getNumberOfVisitors() << " " << this->getVipMemberNumber();
}
