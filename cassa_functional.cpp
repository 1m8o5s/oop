...
void Cassa::sellTickets(Visitor* visitor, Concert* concert, ClubManager* club_manager, SingerManager* singer_manager,  Singer* singer){
	if (this->getNumberOfTickets()){
		if (visitor->getOld() > concert->getAgeLimit()){
			...
			club_manager->distributeMoney(this, singer_manager, singer);
		}
	}
}
...
Cassa::Cassa(int ticket_price, int number_of_tickets, bool end_of_selling, int work_from, int work_to, int cash_percent, int earned_money, char * title)
